...
# BIP39 wordlist (первые 2048 слов стандартного списка)
BIP39_WORDS = [
...
def password_generator(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if not (use_uppercase or use_lowercase or use_digits or use_special):
                use_lowercase = True  # По умолчанию используем строчные буквы
    ...
    context = {
        'passwords': passwords,
        'length': request.POST.get('length', '12') if request.method == 'POST' else '12',
        'uppercase': request.POST.get('uppercase') == 'on' if request.method == 'POST' else True,
        'lowercase': request.POST.get('lowercase') == 'on' if request.method == 'POST' else True,
        'digits': request.POST.get('digits') == 'on' if request.method == 'POST' else True,
        'special': request.POST.get('special') == 'on' if request.method == 'POST' else False,
    }
    
    return render(request, 'generator/password_generator.html', context)


def word_generator(request):
    """Генератор слов для криптокошельков"""
    words = []
    
    if request.method == 'POST':
        try:
            count = int(request.POST.get('count', 12))
            # Генерируем случайные слова из BIP39 wordlist
            words = random.sample(BIP39_WORDS, min(count, len(BIP39_WORDS)))
        except (ValueError, TypeError):
            pass
    
    context = {
        'words': words,
        'count': request.POST.get('count', '12') if request.method == 'POST' else '12',
    }
    
    return render(request, 'generator/word_generator.html', context)
